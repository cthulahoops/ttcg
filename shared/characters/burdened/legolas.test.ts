import { describe, expect, test } from "bun:test";
import { LegolasBurdened } from "./legolas";
import { GameStateBuilder } from "../../test-utils";

describe("Legolas (Burdened)", () => {
  describe("objective.getStatus", () => {
    test("returns { tentative, failure } when no threat card assigned", () => {
      const { game, seats } = new GameStateBuilder(4)
        .setCharacter(0, "Legolas (Burdened)")
        .build();

      expect(LegolasBurdened.objective.getStatus(game, seats[0]!)).toEqual({
        finality: "tentative",
        outcome: "failure",
      });
    });

    test("returns { tentative, failure } when threat card assigned but neither card won", () => {
      const { game, seats } = new GameStateBuilder(4)
        .setCharacter(0, "Legolas (Burdened)")
        .build();

      seats[0]!.threatCard = 5;

      expect(LegolasBurdened.objective.getStatus(game, seats[0]!)).toEqual({
        finality: "tentative",
        outcome: "failure",
      });
    });

    test("returns { tentative, failure } when only forests card won", () => {
      const { game, seats } = new GameStateBuilder(4)
        .setCharacter(0, "Legolas (Burdened)")
        .seatWonCards(0, [{ suit: "forests", value: 5 }])
        .build();

      seats[0]!.threatCard = 5;

      expect(LegolasBurdened.objective.getStatus(game, seats[0]!)).toEqual({
        finality: "tentative",
        outcome: "failure",
      });
    });

    test("returns { tentative, failure } when only shadows card won", () => {
      const { game, seats } = new GameStateBuilder(4)
        .setCharacter(0, "Legolas (Burdened)")
        .seatWonCards(0, [{ suit: "shadows", value: 5 }])
        .build();

      seats[0]!.threatCard = 5;

      expect(LegolasBurdened.objective.getStatus(game, seats[0]!)).toEqual({
        finality: "tentative",
        outcome: "failure",
      });
    });

    test("returns { final, success } when both forests and shadows cards won", () => {
      const { game, seats } = new GameStateBuilder(4)
        .setCharacter(0, "Legolas (Burdened)")
        .seatWonCards(0, [
          { suit: "forests", value: 5 },
          { suit: "shadows", value: 5 },
        ])
        .build();

      seats[0]!.threatCard = 5;

      expect(LegolasBurdened.objective.getStatus(game, seats[0]!)).toEqual({
        finality: "final",
        outcome: "success",
      });
    });

    test("returns { final, success } when both matching cards are among other cards", () => {
      const { game, seats } = new GameStateBuilder(4)
        .setCharacter(0, "Legolas (Burdened)")
        .seatWonCards(0, [
          { suit: "mountains", value: 2 },
          { suit: "forests", value: 3 },
          { suit: "shadows", value: 3 },
        ])
        .build();

      seats[0]!.threatCard = 3;

      expect(LegolasBurdened.objective.getStatus(game, seats[0]!)).toEqual({
        finality: "final",
        outcome: "success",
      });
    });

    test("returns { tentative, failure } when cards of wrong value won", () => {
      const { game, seats } = new GameStateBuilder(4)
        .setCharacter(0, "Legolas (Burdened)")
        .seatWonCards(0, [
          { suit: "forests", value: 3 },
          { suit: "shadows", value: 3 },
        ])
        .build();

      seats[0]!.threatCard = 5;

      expect(LegolasBurdened.objective.getStatus(game, seats[0]!)).toEqual({
        finality: "tentative",
        outcome: "failure",
      });
    });

    test("returns { final, failure } when another player won the forests card", () => {
      const { game, seats } = new GameStateBuilder(4)
        .setCharacter(0, "Legolas (Burdened)")
        .seatWonCards(1, [{ suit: "forests", value: 5 }])
        .build();

      seats[0]!.threatCard = 5;

      expect(LegolasBurdened.objective.getStatus(game, seats[0]!)).toEqual({
        finality: "final",
        outcome: "failure",
      });
    });

    test("returns { final, failure } when another player won the shadows card", () => {
      const { game, seats } = new GameStateBuilder(4)
        .setCharacter(0, "Legolas (Burdened)")
        .seatWonCards(1, [{ suit: "shadows", value: 5 }])
        .build();

      seats[0]!.threatCard = 5;

      expect(LegolasBurdened.objective.getStatus(game, seats[0]!)).toEqual({
        finality: "final",
        outcome: "failure",
      });
    });

    test("returns { final, failure } when forests card is the lost card", () => {
      const { game, seats } = new GameStateBuilder(4)
        .setCharacter(0, "Legolas (Burdened)")
        .withLostCard({ suit: "forests", value: 5 })
        .build();

      seats[0]!.threatCard = 5;

      expect(LegolasBurdened.objective.getStatus(game, seats[0]!)).toEqual({
        finality: "final",
        outcome: "failure",
      });
    });

    test("returns { final, failure } when shadows card is the lost card", () => {
      const { game, seats } = new GameStateBuilder(4)
        .setCharacter(0, "Legolas (Burdened)")
        .withLostCard({ suit: "shadows", value: 5 })
        .build();

      seats[0]!.threatCard = 5;

      expect(LegolasBurdened.objective.getStatus(game, seats[0]!)).toEqual({
        finality: "final",
        outcome: "failure",
      });
    });
  });

  describe("metadata", () => {
    test("has correct name", () => {
      expect(LegolasBurdened.name).toBe("Legolas (Burdened)");
    });

    test("has correct setupText", () => {
      expect(LegolasBurdened.setupText).toBe(
        "Draw a threat card, then exchange with Gimli or Aragorn"
      );
    });

    test("has correct objective text", () => {
      expect(LegolasBurdened.objective.text).toBe(
        "Win both the Forests and Shadows cards matching your threat card"
      );
    });
  });
});
