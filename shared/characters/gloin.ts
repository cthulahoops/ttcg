...
export const Gloin: CharacterDefinition = {
  ...
  setup: async (game, seat, setupContext) => {
    await game.exchange(seat, setupContext, (c: string) => ["Bilbo Baggins", "Gimli"].includes(c));
  },

  objective: {
    text: "Win the most mountains cards",
    check: (game, seat) => {
      const myCounts = seat.getAllWonCards().filter((c: Card) => c.suit === "mountains").length;
      ...
      return game.seats.every((s: Seat) => {
        ...
        const theirCounts = s.getAllWonCards().filter((c: Card) => c.suit === "mountains").length;
    ...
    isCompletable: (game, seat) => {
      ...
      const othersMaxCounts = Math.max(
        ...game.seats
          ...
          .map((s: Seat) => s.getAllWonCards().filter((c: Card) => c.suit === "mountains").length)
      );

      const totalMountainsWon = game.seats.reduce(
        (total: number, s: Seat) =>
          total + s.getAllWonCards().filter((c: Card) => c.suit === "mountains").length,
        0
      );
    ...
    isCompleted: (game, seat) => game.finished && Gloin.objective.check(game, seat),
  },
